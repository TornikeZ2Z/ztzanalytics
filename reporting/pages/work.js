/* IT REQUESTS — one list, nothing else. (V3, his call 2026-08-28: "remove board of
 * kanban too - i just need to see the list of requests, the requestor and date of
 * request with status and its filters". The kanban/tabs/tickets V2 is in git history;
 * the Monday-imported customer-claim tickets were soft-deleted the same day.)
 *
 * Every row is an IT request: filed from the request link (born `Requested`) or created
 * here. The list shows request / requestor / date / status; status chips + search filter
 * it; the drawer holds everything else (status, subtasks, updates, images, edit).
 * Accept = set any working status; Decline asks for a reason the requester sees.
 *
 * /api/_work sits ABOVE the ACL gate (the _hrq precedent): every signed-in employee uses
 * this page, grants or not.
 */
(function () {
  var STATUSES = ["Requested", "Planned", "In Progress", "Stuck", "Done",
                  "Declined", "Dropped"];
  ...
  // the portal colour system's own accents, nothing new — cycled per avatar
  ...
  function fmtDate(v) {
    ...
    return isNaN(d) ? v : d.toLocaleDateString(undefined,
  ...
  function injectStyle() {
    ...
    st.textContent = ""
      ...
      + ".wtx-search{flex:0 1 260px;min-width:170px}"
      ...
      // avatars (drawer)
      + ".wtx-av{width:22px;height:22px;border-radius:99px;display:inline-flex;align-items:"
      + "center;justify-content:center;font-size:9px;font-weight:800;color:#fff;"
  ...
  function render(host) {
    injectStyle();
    host.innerHTML = '<div class="rs-loading" style="padding:22px">Loading requests…</div>';
    var S = { status: "All", q: "", data: null, host: host };
    refresh(S, true);
  }
  ...
  // the tickets (Monday's customer-claim import) are soft-deleted server-side; the
  // filter is a belt on top of that so none ever paints even if one is restored
  function baseItems(S) {
    return S.data.items.filter(function (i) { return i.Kind !== "ticket"; });
  }
  function itemsOf(S) {
    var q = S.q.toLowerCase();
    return baseItems(S).filter(function (i) {
      if (S.status !== "All" && i.Status !== S.status) return false;
      if (q && (i.Title + " " + (i.Assignees || "") + " " + (i["Job No"] || "") + " "
                + (i.Requester || "")).toLowerCase().indexOf(q) < 0) return false;
      return true;
    });
  }
  ...
  function paintShell(S) {
    ...
    var html = '<div class="rs-page-head"><h1>IT Requests</h1>'
      + "<p>Everything IT is asked to build, fix or look at — one list, one history. A "
      + "request filed from the link lands here as <b>Requested</b>; open it to accept, "
      + "decline with a reason, or move it along. The requester can always see where it "
      + "stands.</p></div>"
      + '<div class="wtx-top">'
      + '<input class="wtx-in wtx-search" id="wtxQ" placeholder="search request, person, '
      + 'job #…" value="' + esc(S.q) + '">'
      ...
      + "</div>"
      + '<div class="wtx-chips" id="wtxStatus"></div>'
      + '<div id="wtxBody"></div>';
    ...
    S.host.querySelector("#wtxNew").onclick = function () { openDrawer(S, null, true); };
  ...
  function paintStatusChips(S) {
    var counts = {};
    baseItems(S).forEach(function (i) {
      counts[i.Status] = (counts[i.Status] || 0) + 1;
    });
    var chips = [["All", baseItems(S).length]].concat(STATUSES.filter(function (st) {
      return counts[st];
    }).map(function (st) { return [st, counts[st]]; }));
    var el = S.host.querySelector("#wtxStatus");
    el.innerHTML = chips.map(function (c) {
      return '<span class="wtx-chip' + (S.status === c[0] ? " on" : "") + '" data-st="'
        + esc(c[0]) + '">' + esc(c[0]) + "<small>" + c[1] + "</small></span>";
    }).join("");
    el.querySelectorAll("[data-st]").forEach(function (b) {
      b.onclick = function () { S.status = b.getAttribute("data-st"); paintBody(S); };
    });
  }
  ...
  function paintBody(S) {
    ...
    var items = itemsOf(S);
    if (!items.length) {
      body.innerHTML = '<div class="panel"><div class="wtx-empty">'
        + (S.q || S.status !== "All" ? "Nothing matches these filters."
           : "No requests yet — share the request link and they land here.")
        + "</div></div>";
      return;
    }
    var rows = items.sort(function (a, b) {
      ...
      return '<tr data-open="' + i["Work Id"] + '" style="cursor:pointer">'
        ...
        + "<td>" + esc(fmtDate(i["Entered At"]) || "—") + "</td>"
    ...
    body.innerHTML = '<div class="panel"><div class="rs-tablewrap"><table class="rs-table">'
      + "<thead><tr><th>Request</th><th>Requested by</th><th>Date</th><th>Status</th>"
  ...
  // OPTIMISTIC: flip the row in local state and repaint instantly; the server write runs
  // behind it and any refusal rolls the list back with the reason on a toast.
  // Declining carries the reason the requester sees (the bridge refuses without one).
  function setStatus(S, wid, st, reason) {
    var i = S.data.items.filter(function (x) { return x["Work Id"] === wid; })[0];
    if (!i || i.Status === st) return Promise.resolve();
    var prev = i.Status;
    i.Status = st;
    paintBody(S);
    var payload = { action: "status", work_id: wid, status: st };
  ...
  function openDrawer(S, wid, isNew) {
    var dw = drawerShell(S);
    if (isNew) { paintEdit(S, dw, null); return; }
  ...
  function paintView(S, dw, wid, det) {
    ...
    var strip = STATUSES.map(function (st) {
    ...
    dw.el.querySelectorAll("[data-st]").forEach(function (b) {
      b.onclick = function () {
        ...
        var go = st === "Declined"
          ? RSC.ask({ title: "Decline this request",
                      body: "Say why — the requester sees this reason.",
                      placeholder: "why it is declined…", yes: "Decline" })
              .then(function (why) {
                if (why == null) return null;
                why = why.trim();
                if (why.length < 3) {
                  toast("Say why — the requester sees it", true);
                  return null;
                }
                return setStatus(S, wid, st, why);
              })
          : setStatus(S, wid, st);
        Promise.resolve(go).then(function (res) {
          if (res === null) return;
    ...
    // the warehouse line: a request with a job number explains itself
    if (i["Job No"]) {
  ...
  function paintEdit(S, dw, item) {
    ...
    dw.el.innerHTML = ''
      + '<div class="wtx-dhead"><h2>' + (item ? "Edit" : "New request") + "</h2>"
      + '<button class="rs-btn" id="dwClose">✕</button></div>'
      + '<div class="wtx-form">'
      + '<div class="full"><label>Title</label><input class="wtx-in" id="eTitle" '
      ...
      + '<div><label>Job # (optional)</label><div style="display:flex;gap:6px">'
    ...
    // kit-styled form selects — handles are read at save time.
    // IT-only: the team is not a choice. Existing items keep whatever department they
    // carry; anything edited or created here belongs to Informational Technology.
    var hDept = { get: function () { return v.Department || "Informational Technology"; } };
    var hCat = RSC.localSelect(dw.el.querySelector("#eCatM"), {
      label: "Category", values: d.categories, value: v.Category || "", form: true,
      allLabel: "—",
    });
    var hPrio = RSC.localSelect(dw.el.querySelector("#ePrioM"), {
    ...
    dw.el.querySelector("#eSave").onclick = function () {
      var payload = {
        ...
        category: hCat.get(),
        priority: hPrio.get() || "Medium",
        label: "",
        severity: "",
        job_no: dw.el.querySelector("#eJob").value.trim(),
        start: String(v["Start Date"] || "").slice(0, 10),
